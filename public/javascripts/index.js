$(function () {
    ...
    socket.on('new-tweets', function(tweets) {
        ...
        for (i = 0; i<len; i++) {
            ...
            tweetList.add(t);
            var u = new User({username:twts[i].from_user, id: twts[i].from_user_id, profile_image: twts[i].profile_image_url});
            userList.noDupeAdd(u);
    ...
    socket.emit('getSearch', {term:"louisck"});
});
